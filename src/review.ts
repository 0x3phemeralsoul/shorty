...
export class ReviewService {
  ...
  async handleReviewCommand(interaction: CommandInteraction<CacheType>, discordUserId: string): Promise<void> {
    try {
      ...
      const readyForReviewStories = relevantStories.filter(story => 
        this.shortcutService.isStoryReadyForReview(story)
      );
      ...
      const notReadyStories = relevantStories.filter(story => 
        !this.shortcutService.isStoryReadyForReview(story)
      );
      ...
      for (const story of readyForReviewStories) {
        ...
        if (fullStory.tasks && fullStory.tasks.length > 0) {
          ...
          const hasUserTask = fullStory.tasks.some(task => 
            task.owner_ids && task.owner_ids.includes(shortcutUserId)
          );
