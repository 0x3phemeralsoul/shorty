 ...
 */
export function extractGitHubPullRequestUrls(texts: string[]): string[] {
 ...
 */
export function isGitHubPullRequestUrls(text: string): string[] | null {
  // Define the regex pattern for GitHub PR URLs
  const githubPullRequestRegex = /https?:\/\/github\.com\/[^\/]+\/[^\/]+\/pull\/\d+/g;
 ...
 */
export function getComments(comments: ShortcutComment[]): string[] {
  const results: string[] = [];

  comments.forEach((comment: ShortcutComment) => {
 ...
 */
export function getLastPullRequestUrl(comments: ShortcutComment[]): string {
 ...
 */
export function isCommentWithPullRequest(commentText: string): boolean {
